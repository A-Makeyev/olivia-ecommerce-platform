...
export const createDiscountCode = async (req: any, res: Response, next: NextFunction) => {
  try {
    ...
    if (isTitleExists) {
      return next(new ValidationError('A discount code with this title already exists'))
    }
    ...
    res.status(201).json({ success: true, newDiscountCode })
  } catch (err) {
    return next(err)
  }
}
...
export const deleteDiscountCode = async (req: any, res: Response, next: NextFunction) => {
  try {
    ...
    if (!discountCode) return next(new NotFoundError('Discount code not found'))
    if (discountCode.sellerId !== sellerId) return next(new ValidationError('Not authorized to perform this action'))
...
export const deleteProductImage = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!file_id) return next(new ValidationError('file_id is required'))
      ...
      data: {
        ...
        images: {
          create: images
            ...
            .map((img: any) => ({ file_id: img.file_id, url: img.url }))
        }
      },
      include: { images: true }
    })
...
export const getShopProducts = async (req: any, res: Response, next: NextFunction) => {
  try {
    prisma.products.findMany({
      where: { status: 'Deleted', deletedAt: { lte: new Date() } },
      select: { id: true }
    }).then(async (expired) => {
...
export const deleteProduct = async (req: any, res: Response, next: NextFunction) => {
  try {
    ...
    if (product.status === 'Deleted') return next(new ValidationError('Product is already deleted'))
    ...
    res.status(200).json({
      deletedAt,
      message: `Product scheduled for deletion. Restorable until ${deletedAt.toISOString()}`
    })
  } catch (err) {
    return next(err)
  }
...
export const restoreDeletedProduct = async (req: any, res: Response, next: NextFunction) => {
  try {
    ...
    const product = await prisma.products.findUnique({
      where: { id },
      select: { id: true, shopId: true, status: true, deletedAt: true }
    })

    if (!product) return next(new NotFoundError('Product not found'))
    if (product.shopId !== shopId) return next(new ValidationError('Not authorized to perform this action'))
    if (product.status !== 'Deleted') return next(new ValidationError('Product is not in trash'))

    if (product.deletedAt && product.deletedAt < new Date()) {
      return next(new ValidationError('Restore window has expired'))
    }
...
export const archiveProduct = async (req: any, res: Response, next: NextFunction) => {
  try {
    ...
    if (!product) return next(new NotFoundError('Product not found'))
    if (product.shopId !== shopId) return next(new ValidationError('Not authorized to perform this action'))
    if (product.status === 'Deleted') return next(new ValidationError('Cannot archive a product that is in trash'))
